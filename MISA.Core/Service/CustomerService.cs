        ...
        protected override void CustomValidate(Customer entity, bool isInsert)
        {
            if (entity is Customer)
            {
                ...
                if (!isInsert)
                {
                    ...
                    {
                        throw new CustomerException("CustomerCode" + MISAConstant.Dev_Msg_Exist);
                    }
                    ///Check trùng email
                    var email = _customerRepository.GetEmailById(customer.CustomerId);
                    if (isEmailExist && email != customer.Email)
                    {
                        throw new CustomerException("Email" + MISAConstant.Dev_Msg_Exist);
                    }
                    ///Check trùng SĐT
                    ...
                    {
                        throw new CustomerException("PhoneNumber" + MISAConstant.Dev_Msg_Exist);
                    }
                }
                else
                {
